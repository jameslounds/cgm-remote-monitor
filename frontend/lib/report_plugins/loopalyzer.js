  ...
  daysToShow.forEach(function (day) {
    ...
    if (iobStatusAvailable) {
      ...
      if (daysToShow.length === 1)
        loopalyzer.fillNanWithTreatments(iobs, treatments);
        ...
        while (stopIndex < iobs.length) {
          ...
          if (stopIndex < iobs.length) {
            ...
            if (loopalyzer.canInterpolate(iobs, startIndex, stopIndex)) {
              var k =
                (iobs[stopIndex] - iobs[startIndex]) / (stopIndex - startIndex);
...
loopalyzer.getCOBs = function (
  ...
  daysToShow.forEach(function (day) {
    ...
    if (cobStatusAvailable) {
      ...
      if (daysToShow.length === 1)
        loopalyzer.fillNanWithTreatments(cobs, treatments);
        ...
        while (stopIndex < cobs.length) {
          ...
          if (stopIndex < cobs.length) {
            ...
            if (loopalyzer.canInterpolate(cobs, startIndex, stopIndex)) {
              k =
                (cobs[stopIndex] - cobs[startIndex]) / (stopIndex - startIndex);
              m = cobs[startIndex];
              for (var x = 0; x < stopIndex - startIndex; x++) {
...
loopalyzer.fillNanWithTreatments = function (array, treatments) {
  treatments.forEach(function (treatment) {
    ...
    if (isNaN(array[index])) {
      ...
      var interpolate =
        isNaN(array[start]) || isNaN(array[stop])
          ? true
          : loopalyzer.canInterpolate(array, start, stop);
      if (!interpolate) {
...
loopalyzer.canInterpolate = function (array, start, stop) {
  ...
  if (array[stop] <= array[start] * interpolationRatio) {
    ...
    // Rising
    if (stop - start < risingInterpolationGap && array[start] !== 0)
      interpolate = true;
  }
  return interpolate;
};
...
loopalyzer.getAllTreatmentTimestampsForADay = function (datastorage, day) {
  ...
  insulinTreatments.forEach(function (entry) {
    timestamps.push(entry.date);
  });
...
loopalyzer.getAllPredictionsForADay = function (datastorage, day) {
  ...
  for (var i = datastorage.devicestatus.length - 1; i >= 0; i--) {
    if (
      datastorage.devicestatus[i].loop &&
      datastorage.devicestatus[i].loop.predicted
    ) {
      ...
      datastorage.devicestatus[i].openaps &&
      datastorage.devicestatus[i].openaps.suggested &&
      datastorage.devicestatus[i].openaps.suggested.predBGs
    ) {
      var entry = {};
      ...
      if (datastorage.devicestatus[i].openaps.suggested.predBGs.COB) {
        entry.values =
          datastorage.devicestatus[i].openaps.suggested.predBGs.COB;
      } else if (datastorage.devicestatus[i].openaps.suggested.predBGs.UAM) {
        entry.values =
          datastorage.devicestatus[i].openaps.suggested.predBGs.UAM;
      } else
        entry.values =
          datastorage.devicestatus[i].openaps.suggested.predBGs.IOB;
...
loopalyzer.getPredictions = function (datastorage, daysToShow, client) {
  if (!datastorage.devicestatus) return [];
  ...
  daysToShow.forEach(function (day) {
    ...
    if (predictions.length > 0 && treatmentTimestamps.length > 0) {
      ...
      for (
        ...
        treatmentsIndex < treatmentTimestamps.length;
        treatmentsIndex++
      ) {
...
loopalyzer.addArrayToBins = function (bins, values) {
  ...
  } else console.log("addArrayToBins - array must have 288 items", values);
};
...
loopalyzer.renderProfilesTable = function (
  ...
  daysToShow.forEach(function (day) {
    ...
    if (!endOfLastDay || dayEnd > endOfLastDay) endOfLastDay = dayEnd;
  });
    ...
    if (store) {
      for (var key in store) {
        ...
        if (Object.prototype.hasOwnProperty.call(store, key)) {
          ...
          if (defaultProfile.carbratio)
            newEntry.carbratio = defaultProfile.carbratio;
          if (defaultProfile.sens) newEntry.sens = defaultProfile.sens;
          if (
            (newEntry.basal || newEntry.carbratio || newEntry.sens) &&
            moment(entry.startDate).isBefore(endOfLastDay)
          )
  ...
  profilesArray1.sort(function (a, b) {
...
loopalyzer.report = function (datastorage, sorteddaystoshow, options) {
  ...
  daysToShow.sort(function (a, b) {
    return a < b ? -1 : 1;
  }); // We always want them chronological order
...
loopalyzer.generateReport = function (datastorage, daysToShow, options) {
  ...
  if (
    $("#rp_loopalyzerprofiles").is(":checked") &&
    datastorage.profiles &&
    datastorage.profiles.length > 0
  ) {
    $("#loopalyzer-profiles-table").show();
    loopalyzer.renderProfilesTable(datastorage.profiles, daysToShow, client);
  } else $("#loopalyzer-profiles-table").hide();
  ...
  if ($("#rp_loopalyzertimeshift").is(":checked") && daysToShow.length > 1) {
    ...
    if (t2 > t1) {
      ...
      daysToShow.forEach(function (day, dayIndex) {
        ...
        carbTreatments.forEach(function (entry) {
          if (!found && entry.amount >= mealMinCarbs) {
            ...
            if (
              (date.isSame(timeShiftBegin, "minute") ||
                date.isAfter(timeShiftBegin, "minute")) &&
              (date.isSame(timeShiftEnd, "minute") ||
                date.isBefore(timeShiftEnd, "minute"))
            ) {
      ...
      if (!dia || dia <= 0) dia = 6; // Default to 6h if DIA not set in profile
      ...
      else timeShiftStopTime.minutes(24 * 60 - 1); // If beyond midnight, stop at midnight
  ...
  if (doTimeShift)
    markings.push({
      xaxis: {
        from: timeShiftStartTime.toDate(),
        to: timeShiftStopTime.toDate(),
      },
      color: timeShiftBackgroundColor,
    });
  var chartBasalData = [
    {
      data: basalsAvg,
  ...
  insulinTreatments.forEach(function (treatment) {
    ...
    markings.push({
      xaxis: { from: startDate.toDate(), to: endDate.toDate() },
      yaxis: { from: 0, to: treatment.amount },
      color: markingColor,
    });
  });
