...
function uniqBasedOnMills(a) {
  ...
  return a.filter(function (item) {
    return Object.prototype.hasOwnProperty.call(seen, item.mills)
      ? false
      : (seen[item.mills] = true);
  });
}
...
const processForRuntime = (obj) => {
  Object.keys(obj).forEach((key) => {
    if (typeof obj[key] === "object" && obj[key]) {
      ...
      if (
        obj[key].hasOwnProperty("created_at") &&
        !obj[key].hasOwnProperty("mills")
      ) {
      ...
      Object.keys(obj).forEach((key) => {
        if (typeof obj[key] === "object" && obj[key]) {
          ...
          if (
            ...
            !element.hasOwnProperty("absolute")
          ) {
...
function loadEntries(ddata, ctx, callback) {
  ...
  const loadTime =
    ctx.cache.isEmpty("entries") || withFrame
      ? longLoad
...
function loadTreatments(ddata, ctx, callback) {
  ...
  const loadTime =
    ctx.cache.isEmpty("treatments") || withFrame
      ? longLoad
      : constants.FIFTEEN_MINUTES;
  ...
  ctx.treatments.list(tq, function (err, results) {
    if (!err && results) {
      ...
      ddata.treatments = ctx.ddata.idMergePreferNew(
        ddata.treatments,
        currentData,
      );
    }
...
function loadProfileSwitchTreatments(ddata, ctx, callback) {
  var dateRange = {
    $gte: new Date(
      ddata.lastUpdated - constants.ONE_DAY * 31 * 12,
    ).toISOString(),
  };
...
function loadDeviceStatus(ddata, env, ctx, callback) {
  ...
  const longLoad =
    ...
    env.extendedSettings.devicestatus.days == 2
      ...
      : constants.ONE_DAY;
  const loadTime =
    ctx.cache.isEmpty("devicestatus") || withFrame
      ? longLoad
      : constants.FIFTEEN_MINUTES;
