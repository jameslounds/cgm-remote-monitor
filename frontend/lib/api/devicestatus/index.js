...
function configure(app, wares, ctx, env) {
  ...
  function processDates(results) {
    ...
    if (env.settings.deNormalizeDates) {
      ...
      results.forEach(function (e) {
        if (
          e.created_at &&
          Object.prototype.hasOwnProperty.call(e, "utcOffset")
        ) {
  ...
  api.get("/devicestatus/", function (req, res) {
    ...
    const canServeFromMemory =
      inMemoryData.length >= q.count && Object.keys(q).length == 1
        ? true
        : false;
  ...
  function config_authed(app, api, wares, ctx) {
    ...
    api.post(
      "/devicestatus/",
      ctx.authorization.isPermitted("api:devicestatus:create"),
      doPost,
    );
    ...
    api.delete(
      "/devicestatus/:id",
      ctx.authorization.isPermitted("api:devicestatus:delete"),
