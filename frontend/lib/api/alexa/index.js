...
function configure(app, wares, ctx, env) {
      ...
      switch (req.body.request.type) {
        ...
        case "LaunchRequest":
          if (!req.body.request.intent) {
            onLaunch(function () {
              res.json(
                ctx.alexa.buildSpeechletResponse(
        ...
        case "IntentRequest": // eslint-disable-line no-fallthrough
          onIntent(req.body.request.intent, function (title, response) {
            res.json(
              ctx.alexa.buildSpeechletResponse(title, response, "", true),
            );
            next();
          });
          break;
  ...
  function handleIntent(intentName, slots, next) {
    var metric;
    if (slots) {
      var slotStatus = _.get(
        slots,
        "metric.resolutions.resolutionsPerAuthority[0].status.code",
      );
      var slotName = _.get(
        slots,
        "metric.resolutions.resolutionsPerAuthority[0].values[0].value.name",
      );
      if (slotStatus == "ER_SUCCESS_MATCH" && slotName) {
        metric = slotName;
      } else {
        next(
          translate("virtAsstUnknownIntentTitle"),
          translate("virtAsstUnknownIntentText"),
        );
        return;
      }
    }
