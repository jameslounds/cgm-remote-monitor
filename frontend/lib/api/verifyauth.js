...
function configure(ctx) {
  ...
  api.get("/verifyauth", function (req, res) {
    ctx.authorization.resolveWithRequest(req, function resolved(err, result) {
      ...
      var canRead =
        !err && ctx.authorization.checkMultiple("*:*:read", result.shiros);
      var canWrite =
        !err && ctx.authorization.checkMultiple("*:*:write", result.shiros);
      var isAdmin =
        !err && ctx.authorization.checkMultiple("*:*:admin", result.shiros);
