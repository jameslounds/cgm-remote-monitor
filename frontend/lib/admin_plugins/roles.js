  ...
  $.ajax({
    ...
    .done(function success(records) {
      ...
      $status
        .hide()
        .text(
          client.translate("Database contains %1 roles", {
...
function genDialog(client) {
  var ret =
    '<div id="editroledialog" style="display:none" title="' +
    client.translate("Edit Role") +
    '">' +
    '      <label for="edrole_name">' +
    client.translate("Name") +
    '          <input id="edrole_name" placeholder="' +
    client.translate("admin, school, family, etc") +
    '"/>' +
    "      </label>" +
    "      <br>" +
    '      <label for="edrole_permissions">' +
    client.translate("Permissions") +
    "</label>" +
    '      <textarea id="edrole_permissions" rows="3" style="width:300px"></textarea><br>' +
    "      <br>" +
    '      <label for="edrole_notes">' +
    ...
    buttons: [
      {
        ...
        click: function () {
          ...
          role.permissions = _.chain(
            $("#edrole_permissions").val().toLowerCase().split(/[;, ]/),
          )
            .map(_.trim)
            .reject(_.isEmpty)
            .sort()
...
function showRole(role, table, client) {
  var editIcon = $(
    '<img title="' +
      client.translate("Edit this role") +
    ...
    $("<tr>")
      ...
      .append(
        $("<td>")
          .attr("width", "20%")
          .append(
            _.isEmpty(role.permissions) ? "[none]" : role.permissions.join(" "),
          ),
      )
      .append(
        $("<td>")
          .attr("width", "10%")
          .append(
            role._id ? (role.notes ? role.notes : "") : "[system default]",
          ),
      ),
  );
}
