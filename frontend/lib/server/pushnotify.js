...
function init(env, ctx) {
  ...
  var receipts = new NodeCache({
    stdTTL: times.hour().secs,
    checkperiod: times.mins(5).secs,
  });
  var recentlySent = new NodeCache({
    stdTTL: times.mins(15).secs,
    checkperiod: 20,
  });
  ...
  pushnotify.pushoverAck = function pushoverAck(response) {
    ...
    if (notify) {
      ...
      ctx.notifications.ack(
        notify.level,
        notify.group,
        times.mins(snoozeMins).msecs,
        true,
      );
      receipts.del(response.receipt);
    } else {
      console.info(
        "unable to find notify for pushover ack",
        response,
        receipts.keys(),
      );
    }
    ...
    if (ctx.pushover) {
      ...
      _.each(receiptKeys, function eachKey(receipt) {
        ctx.pushover.cancelWithReceipt(receipt, function cancelCallback(err) {
          if (err) {
            console.error(
              "error canceling receipt:" + receipt + ", err: ",
              err,
            );
          } else {
            console.info(
              "got a receipt cancel response for:" +
                receipt +
                ", removing from cache",
            );
