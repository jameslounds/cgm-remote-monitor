...
function create(env, ctx) {
  ...
  if (!insecureUseHttp) {
    console.info(
      "Redirecting http traffic to https because INSECURE_USE_HTTP=",
      insecureUseHttp,
    );
    ...
    if (secureHstsHeader) {
      ...
      if (enableCSP) {
        ...
        if (secureCspReportOnly) {
          console.info(
            "Enabled SECURE_CSP (Content Security Policy header). Not enforcing. Report only.",
          );
        } else {
          console.info(
            "Enabled SECURE_CSP (Content Security Policy header). Enforcing.",
          );
        }
        ...
        cspPolicy = {
          //TODO make NS work without 'unsafe-inline'
          directives: {
            defaultSrc: ["'self'"],
            styleSrc: [
              "'self'",
    ...
    console.info(
      "Security settings: INSECURE_USE_HTTP=",
      insecureUseHttp,
      ", SECURE_HSTS_HEADER=",
      secureHstsHeader,
    );
  ...
  let cacheBuster =
    process.env.NODE_ENV == "development" ? "developmentMode" : randomToken(16);
  ...
  const swcontent = fs.readFileSync(resolvePath("/views/service-worker.js"), {
    encoding: "utf-8",
  });
  ...
  if (process.env.NODE_ENV === "development") {
    maxAge = 1;
    console.log(
      "Development environment detected, setting static file cache age to 1 second",
    );
  }
  ...
  app.use(
    "/translations",
    express.static(resolvePath("/translations"), {
      maxAge,
    }),
  );
  ...
  const swaggerjson = fs.readFileSync(
    resolvePath(__dirname + "/swagger.json"),
    { encoding: "utf-8" },
  );
  const swaggeryaml = fs.readFileSync(
    resolvePath(__dirname + "/swagger.yaml"),
    { encoding: "utf-8" },
  );
  ...
  app.use(
    "/api3-docs",
    swaggerUi.serve,
    swaggerUseSchema(swaggerDocumentApiV3),
  );
  ...
  // Production bundling
  const tmpFiles = express.static(
    resolvePath("/node_modules/.cache/_ns_cache/public"),
    {
      maxAge: maxAge,
    },
  );
