...
function setSSL() {
  ...
  env.secureHstsHeaderIncludeSubdomains = readENVTruthy(
    "SECURE_HSTS_HEADER_INCLUDESUBDOMAINS",
    false,
  );
  env.secureHstsHeaderPreload = readENVTruthy(
...
function setAPISecret() {
  ...
  if (useSecret) {
    if (readENV("API_SECRET").length < consts.MIN_PASSPHRASE_LENGTH) {
      var msg = [
        "API_SECRET should be at least",
        consts.MIN_PASSPHRASE_LENGTH,
        "characters",
      ].join(" ");
      console.error(msg);
      ...
      if (testresult < 60) {
        env.notifies.push({
          ...
          message:
            "Weak API_SECRET detected. Please use a mix of small and CAPITAL letters, numbers and non-alphanumeric characters such as !#%&/ to reduce the risk of unauthorized access. The minimum length of the API_SECRET is 12 characters.",
        });
      }
      ...
      if (env.storageURI) {
        ...
        if (parsedURL.auth && parsedURL.auth.password == apiSecret) {
          env.notifies.push({
            ...
            title: "Security issue",
            message:
              "MongoDB password and API_SECRET match. This is a really bad idea. Please change both and do not reuse passwords across the system.",
          });
        }
      }
    }
...
function setStorage() {
  env.storageURI =
    readENV("STORAGE_URI") ||
    readENV("MONGO_CONNECTION") ||
    readENV("MONGO") ||
    readENV("MONGOLAB_URI") ||
    readENV("MONGODB_URI");
  env.entries_collection =
    readENV("ENTRIES_COLLECTION") || readENV("MONGO_COLLECTION", "entries");
  env.authentication_collections_prefix = readENV(
    "MONGO_AUTHENTICATION_COLLECTIONS_PREFIX",
    "auth_",
  );
  env.treatments_collection = readENV(
    "MONGO_TREATMENTS_COLLECTION",
    "treatments",
  );
  env.profile_collection = readENV("MONGO_PROFILE_COLLECTION", "profile");
  env.settings_collection = readENV("MONGO_SETTINGS_COLLECTION", "settings");
  env.devicestatus_collection = readENV(
...
function readENV(varName, defaultValue) {
  ...
  var value =
    shadowEnv["CUSTOMCONNSTR_" + varName] ||
...
function findExtendedSettings(envs) {
  ...
  if (shadowEnv["DEVICESTATUS_DAYS"] && shadowEnv["DEVICESTATUS_DAYS"] == "2")
    extended.devicestatus.days = 1;
