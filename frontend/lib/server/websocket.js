...
function init(env, ctx, server) {
  ...
  function status() {
    ...
    if (verParse) {
      versionNum =
        10000 * Number(verParse[0]) +
        100 * Number(verParse[1]) +
        1 * Number(verParse[2]);
    }
    ...
    var info = {
      ...
      careportalEnabled:
        apiEnabled && env.settings.enable.indexOf("careportal") > -1,
      boluscalcEnabled:
        apiEnabled && env.settings.enable.indexOf("boluscalc") > -1,
      settings: env.settings,
      extendedSettings:
        ctx.plugins && ctx.plugins.extendedClientSettings
          ? ctx.plugins.extendedClientSettings(env.extendedSettings)
          : {},
    };
  ...
  function verifyAuthorization(message, ip, callback) {
    ...
    ctx.authorization.resolve(
      { api_secret: message.secret, token: message.token, ip: ip },
      function resolved(err, result) {
        ...
        return callback(null, {
          ...
          write: ctx.authorization.checkMultiple(
            "api:*:create,update,delete",
            result.shiros,
          ),
          write_treatment: ctx.authorization.checkMultiple(
  ...
  function listeners() {
    io.sockets.on("connection", function onConnection(socket) {
      ...
      console.log(
        LOG_WS + "Connection from client ID: ",
        socket.client.id,
        " IP: ",
        remoteIP,
      );
      ...
      function checkConditions(action, data) {
        ...
        if (action === "dbUpdate" && !data._id) {
          console.log(
            "WS dbUpdate/dbAddnot sure abou documentati call: ",
            "Missing _id",
            data,
          );
      ...
      socket.on("loadRetro", function loadRetro(opts, callback) {
        ...
        socket
          .compress(true)
          .emit("retroUpdate", { devicestatus: lastData.devicestatus });
      ...
      socket.on("dbUpdate", function dbUpdate(data, callback) {
        ...
        ctx.store
          ...
          .update({ _id: id }, { $set: data.data }, function (err, results) {
            if (!err) {
              ctx.store
                .collection(collection)
                .findOne({ _id: id }, function (err, results) {
      ...
      socket.on("dbUpdateUnset", function dbUpdateUnset(data, callback) {
        console.log(
          LOG_WS + "dbUpdateUnset client ID: ",
          socket.client.id,
          " data: ",
          data,
        );
          ...
          .update(
            ...
            function (err, results) {
              if (!err) {
                ctx.store
                  .collection(collection)
                  .findOne({ _id: objId }, function (err, results) {
                    console.log("Got results", results);
                    if (!err && results !== null) {
                      ctx.bus.emit("data-update", {
                        type: data.collection,
        ...
        if (data.collection === "treatments") {
          ...
          ctx.store
            ...
            .toArray(function findResult(err, array) {
              ...
              var query_similiar = {
                created_at: {
                  $gte: new Date(
                    new Date(data.data.created_at).getTime() - maxtimediff,
                  ).toISOString(),
                  $lte: new Date(
                    new Date(data.data.created_at).getTime() + maxtimediff,
                  ).toISOString(),
                  ...
                  ctx.store
                    ...
                    .insert(data.data, function insertResult(err, doc) {
                      if (err != null && err.message) {
                        console.log(
                          "treatments data insertion error: ",
                          err.message,
                        );
                        return;
                      }
          ...
          ctx.store
            ...
            .insert(data.data, function insertResult(err, doc) {
              if (err != null && err.message) {
                console.log(
                  data.collection + " insertion error: ",
                  err.message,
                );
                return;
              }
      ...
      socket.on("dbRemove", function dbRemove(data, callback) {
        ...
        ctx.store
          .collection(collection)
          .remove({ _id: objId }, function (err, stat) {
            if (!err) {
              ctx.bus.emit("data-update", {
                type: data.collection,
                op: "remove",
      ...
      socket.on("authorize", function authorize(message, callback) {
        ...
        verifyAuthorization(
          message,
          remoteIP,
          function verified(err, authorization) {
            if (err) {
