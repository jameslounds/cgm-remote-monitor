...
function boot(env, language) {
  ...
  function checkNodeVersion(ctx, next) {
    ...
    if (
      ...
      (semver.satisfies(nodeVersion, "^20.0.0") ||
        semver.satisfies(nodeVersion, "^18.0.0") ||
    ...
    console.log(
      "ERROR: Node version " +
        nodeVersion +
        " is not supported. Please use a secure LTS version or upgrade your Node",
    );
    process.exit(1);
  }
      ...
      return axios
        ...
        .then(function (resp) {
          ...
          if (body.extendedSettings) {
            console.log(
              "extending extendedSettings with",
              body.extendedSettings,
            );
  ...
  function checkSettings(ctx, next) {
    ...
    if (!env.storageURI) {
      ctx.bootErrors.push({
        desc: "Mandatory setting missing",
      ...
      if (_.startsWith(env.storageURI, "openaps://")) {
        ...
        require("../storage/mongo-storage")(env, function ready(err, store) {
          ...
          if (err) {
            ...
            ctx.bootErrors = ctx.bootErrors || [];
            ctx.bootErrors.push({
              desc: "Unable to connect to Mongo",
              err: err.message,
            });
          }
  ...
  function setupAuthorization(ctx, next) {
    ...
    ctx.authorization.storage.reload(function loaded(err) {
      if (err) {
        ctx.bootErrors = ctx.bootErrors || [];
        ctx.bootErrors.push({
          desc: "Unable to setup authorization",
          err: err,
        });
      }
      next();
  ...
  function setupInternals(ctx, next) {
    ...
    ctx.devicestatus = require("./devicestatus")(
    ...
    if (
      env.settings.isEnabled("alexa") ||
      env.settings.isEnabled("googlehome")
    ) {
  ...
  function setupBridge(ctx, next) {
    ...
    if (ctx.bridge) {
      ...
      console.log(
        "DEPRECATION WARNING",
        "PLEASE CONSIDER nightscout-connect instead.",
      );
    }
    next();
  }
  ...
  function setupMMConnect(ctx, next) {
    ...
    ctx.mmconnect = require("../plugins/mmconnect").init(
      env,
      ctx.entries,
      ctx.devicestatus,
      ctx.bus,
