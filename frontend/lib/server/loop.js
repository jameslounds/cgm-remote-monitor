...
function init(env, ctx) {
  ...
  loop.sendNotification = function sendNotification(
    data,
    remoteAddress,
    completion,
  ) {
    ...
    if (
      env.extendedSettings.loop.apnsKey === undefined ||
      env.extendedSettings.loop.apnsKey.length == 0
    ) {
    ...
    if (
      env.extendedSettings.loop.apnsKeyId === undefined ||
      env.extendedSettings.loop.apnsKeyId.length == 0
    ) {
    ...
    if (
      env.extendedSettings.loop.developerTeamId === undefined ||
      env.extendedSettings.loop.developerTeamId.length != 10
    ) {
    ...
    if (
      ctx.ddata.profiles === undefined ||
      ctx.ddata.profiles.length < 1 ||
      ctx.ddata.profiles[0].loopSettings === undefined
    ) {
      completion(
    ...
    if (data.eventType === "Temporary Override Cancel") {
      ...
      if (payload["carbs-entry"] > 0.0) {
        ...
        if (
          data.remoteAbsorption !== undefined &&
          parseFloat(data.remoteAbsorption) > 0.0
        ) {
    ...
    provider
      .send(notification, [loopSettings.deviceToken])
      .then((response) => {
        if (response.sent && response.sent.length > 0) {
          ...
          if (response.failed && response.failed.length > 0) {
            ...
            const reason =
              failedResponse.response && failedResponse.response.reason
