...
function addExtraData(first, req, sbx) {
  ...
  function addBattery() {
    ...
    var battery =
      uploaderStatus &&
      uploaderStatus.uploader &&
      uploaderStatus.uploader.battery;
...
function configure(env, ctx) {
  ...
  return [
    middle,
    wares.sendJSONStatus,
    ctx.authorization.isPermitted("api:pebble,entries:read"),
    pebble,
  ];
}
