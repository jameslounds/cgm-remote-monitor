...
function configure(env, ctx) {
  ...
  self.setENVTruthy = function setENVTruthy(varName, defaultValue) {
    ...
    let value =
      process.env["CUSTOMCONNSTR_" + varName] ||
  ...
  self.setupApiEnvironment = function setupApiEnvironment() {
    app.use(
      ...
      function errorHandler(err, req, res, next) {
        ...
        if (next) {
          // we need 4th parameter next to behave like error handler, but we have to use it to prevent "unused variable" message
        }
      },
    );
    ...
    self.setENVTruthy(
      ...
      apiConst.API3_DEDUP_FALLBACK_ENABLED,
    );
    self.setENVTruthy(
      "API3_CREATED_AT_FALLBACK_ENABLED",
  ...
  self.setupApiRoutes = function setupApiRoutes() {
    ...
    if (app.get("env") === "development" || app.get("ci")) {
  ...
  app.use((req, res) => {
    opTools.sendJSONStatus(
      res,
      apiConst.HTTP.NOT_FOUND,
      apiConst.MSG.HTTP_404_BAD_OPERATION,
    );
  });
