...
function parseFilter(filterDef, logicalOperator, onlyValid) {
  ...
  for (const itemDef of filterDef) {
    ...
    switch (itemDef.operator) {
      ...
      default:
        throw new Error(
          "Unsupported or missing filter operator " + itemDef.operator,
        );
    }
...
function identifyingFilter(identifier, doc, dedupFallbackFields) {
  ...
  if (
    !_.isEmpty(doc) &&
    _.isArray(dedupFallbackFields) &&
    dedupFallbackFields.length > 0
  ) {
    ...
    if (dedupFilterItems.length === dedupFallbackFields.length) {
      // all dedup fields are present
  ...
  if (filterItems.length > 0) return { $or: filterItems };
  else return null; // we don't have any filtering rule to identify the document in the storage
}
