    ...
    if (!token) {
      return reject(
        opTools.sendJSONStatus(
          res,
          apiConst.HTTP.UNAUTHORIZED,
          apiConst.MSG.HTTP_401_MISSING_OR_BAD_TOKEN,
        ),
      );
    }

    ctx.authorization.resolve(
      { token, ip: getRemoteIP(req) },
      function resolveFinish(err, result) {
        if (err) {
          return reject(
            opTools.sendJSONStatus(
              res,
              apiConst.HTTP.UNAUTHORIZED,
              apiConst.MSG.HTTP_401_BAD_TOKEN,
            ),
          );
        } else {
  ...
  return new Promise(function promise(resolve, reject) {
    ...
    if (checkPermission(auth, permission)) {
      ...
      return reject(
        opTools.sendJSONStatus(
          res,
          apiConst.HTTP.FORBIDDEN,
          apiConst.MSG.HTTP_403_MISSING_PERMISSION.replace("{0}", permission),
        ),
      );
    }
  });
