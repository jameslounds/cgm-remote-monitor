...
async function patch(opCtx) {
  ...
  if (_.isEmpty(doc)) {
    return opTools.sendJSONStatus(
      res,
      apiConst.HTTP.BAD_REQUEST,
      apiConst.MSG.HTTP_400_BAD_REQUEST_BODY,
    );
  }
  ...
  if (result.length > 0) {
    ...
    if (
      ifUnmodifiedSince &&
      dateTools.floorSeconds(modifiedDate) >
        dateTools.floorSeconds(new Date(ifUnmodifiedSince))
    ) {
...
async function applyPatch(opCtx, identifier, doc, storageDoc) {
  ...
  if (validate(opCtx, doc, storageDoc) !== true) return;
  ...
  if (!matchedCount) throw new Error("matchedCount empty");
  ...
  return async function operation(req, res) {
    ...
    try {
      ...
      if (!res.headersSent) {
        return opTools.sendJSONStatus(
          res,
          apiConst.HTTP.INTERNAL_ERROR,
          apiConst.MSG.STORAGE_ERROR,
        );
      }
    }
  };
