    ...
    if (match != null) {
      ...
      if (!operators.includes(operator)) {
        opTools.sendJSONStatus(
          ...
          apiConst.MSG.HTTP_400_UNSUPPORTED_FILTER_OPERATOR.replace(
            "{0}",
            operator,
          ),
        );
...
function parseSort(req, res) {
  ...
  if (req.query.sort && req.query.sort$desc) {
    opTools.sendJSONStatus(
      res,
      apiConst.HTTP.BAD_REQUEST,
      apiConst.MSG.HTTP_400_SORT_SORT_DESC,
    );
    return null;
  }
  ...
  if (req.query.skip) {
    if (!isNaN(req.query.skip) && req.query.skip >= 0) {
      ...
      opTools.sendJSONStatus(
        res,
        apiConst.HTTP.BAD_REQUEST,
        apiConst.MSG.HTTP_400_BAD_SKIP,
      );
      return null;
    }
  }
