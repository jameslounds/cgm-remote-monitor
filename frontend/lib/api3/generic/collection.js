...
function Collection({
  ...
  self.dedupFallbackFields = app.get("API3_DEDUP_FALLBACK_ENABLED")
    ? dedupFallbackFields
    : [];
  self.autoPruneDays = app.setENVTruthy(
    "API3_AUTOPRUNE_" + colName.toUpperCase(),
  );
  ...
  self.mapRoutes = function mapRoutes() {
    ...
    // GET /{collection}/history
    app.get(
      prefixHistory + "/:lastModified",
      historyOperation(ctx, env, app, self),
    );
  ...
  self.autoPrune = function autoPrune() {
    if (!stringTools.isNumberInString(self.autoPruneDays)) return;
    ...
    if (new Date() > self.nextAutoPrune) {
      const deleteBefore = new Date(
        new Date().getTime() - autoPruneDays * 24 * 3600 * 1000,
      );
      ...
      const filter = [
        { field: "srvCreated", operator: "lt", value: deleteBefore.getTime() },
        {
          field: "created_at",
          operator: "lt",
          value: deleteBefore.toISOString(),
        },
