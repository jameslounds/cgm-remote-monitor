  ...
  self.subscribe = function subscribe(socket, message, returnCallback) {
    ...
    if (message && message.accessToken) {
      return ctx.authorization.resolveAccessToken(
        message.accessToken,
        function resolveFinishForToken(err, auth) {
          if (err) {
            console.log(
            ...
            socket.on("ack", function onAck(level, group, silenceTime) {
              ...
              console.info(
                LOG + "ack received " + level + " " + group + " " + silenceTime,
              );
            });
    ...
    if (message && shouldTry) {
      return ctx.authorization.resolve(
        ...
        function resolveFinish(err, auth) {
          if (err) {
            console.log(
              `${LOG_ERROR} Authorization failed for jwtToken:`,
              message.jwtToken,
            );
            ...
            var perms = {
              ...
              ack: ctx.authorization.checkMultiple(
                "notifications:*:ack",
                auth.shiros,
              ),
            };
