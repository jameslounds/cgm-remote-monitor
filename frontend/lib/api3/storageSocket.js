...
function StorageSocket(app, env, ctx) {
  ...
  self.subscribe = function subscribe(socket, message, returnCallback) {
    ...
    if (message && message.accessToken) {
      return ctx.authorization.resolveAccessToken(
        message.accessToken,
        function resolveFinish(err, auth) {
          if (err) {
            console.log(
              `${LOG_ERROR} Authorization failed for accessToken:`,
              message.accessToken,
            );
            ...
            if (shouldCallBack) {
              returnCallback({
                success: false,
                message: apiConst.MSG.SOCKET_MISSING_OR_BAD_ACCESS_TOKEN,
              });
            }
            return err;
          } else {
            return self.subscribeAuthorized(
              socket,
              message,
              auth,
              returnCallback,
   ...
   * @param {Function} returnCallback function for returning a value back to the client
   */
  self.subscribeAuthorized = function subscribeAuthorized(
    socket,
    message,
    auth,
    returnCallback,
    ...
    const cols = Array.isArray(message.collections)
      ? message.collections
      : enabledCols;
    const subscribed = [];
    ...
    for (const col of cols) {
      if (enabledCols.includes(col)) {
        const permission =
          col === "settings" ? `api:${col}:admin` : `api:${col}:read`;
  ...
  self.emitCreate = function emitCreate(event) {
    self.namespace.to(event.colName).emit("create", event);
  };
  ...
  self.emitUpdate = function emitUpdate(event) {
    self.namespace.to(event.colName).emit("update", event);
  };
  ...
  self.emitDelete = function emitDelete(event) {
    self.namespace.to(event.colName).emit("delete", event);
  };
}
