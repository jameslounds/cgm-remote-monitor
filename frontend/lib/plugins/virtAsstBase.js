...
function init(env, ctx) {
  ...
  virtAsstBase.setupMutualIntents = function (configuredPlugin) {
    // full status
    configuredPlugin.addToRollup(
      "Status",
      function (slots, sbx, callback) {
    ...
    configuredPlugin.configureIntentHandler(
      "NSStatus",
      function (callback, slots, sbx, locale) {
        configuredPlugin.getRollup(
          "Status",
          sbx,
          slots,
          locale,
    ...
    ctx.plugins.eachEnabledPlugin(function (plugin) {
      if (plugin.virtAsst) {
        if (plugin.virtAsst.intentHandlers) {
          console.log(
            'Plugin "' + plugin.name + '" supports Virtual Assistants',
          );
        ...
        if (plugin.virtAsst.rollupHandlers) {
          console.log(
            'Plugin "' +
              plugin.name +
              '" supports rollups for Virtual Assistants',
          );
        ...
        console.log(
          'Plugin "' + plugin.name + '" does not support Virtual Assistants',
        );
      }
    });
  };
