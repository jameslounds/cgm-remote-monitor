...
function init(ctx) {
  ...
  bwp.checkNotifications = function checkNotifications(sbx) {
    ...
    if (bwp.highSnoozedByIOB(prop, settings, sbx)) {
      ...
      prop.scaledSGV > sbx.data.profile.getHighBGTarget(sbx.time) &&
      prop.bolusEstimate > settings.warnBWP
    ) {
      var level =
        prop.bolusEstimate > settings.urgentBWP ? levels.URGENT : levels.WARN;
  ...
  bwp.highSnoozedByIOB = function highSnoozedByIOB(prop, settings, sbx) {
    ...
    var high =
      ar2EventType === "high" ||
      prop.scaledSGV >= sbx.scaleMgdl(sbx.settings.thresholds.bgTargetTop);
  ...
  bwp.calc = function calc(sbx) {
    ...
    var recentCarbs = _.findLast(
      sbx.data.treatments,
      function eachTreatment(treatment) {
        return (
    ...
    if (prop && prop.errors) {
      ...
      if (prop.bolusEstimateDisplay < 0) {
        ...
        info.unshift({
          ...
          value: "1U / " + sbx.data.profile.getCarbRatio() + " g",
        });
        ...
        if (prop.recentCarbs) {
          info.unshift({
            ...
            value:
              prop.recentCarbs.carbs +
              "g @ " +
              new Date(prop.recentCarbs.mills).toLocaleTimeString([], {
                hour: "2-digit",
                minute: "2-digit",
              }),
          });
  ...
  function pushTempBasalAdjustments(prop, info, sbx) {
    if (prop && prop.tempBasalAdjustment) {
      ...
      info.push({
        ...
        value: sbx.data.profile.getBasal(sbx.time),
      });
      ...
      if (
        ...
        info.push({
          ...
          value:
            "" +
            prop.tempBasalAdjustment.thirtymin +
            "% (" +
            sign +
      ...
      if (
        ...
        info.push({
          ...
          value:
            "" +
            prop.tempBasalAdjustment.onehour +
            "% (" +
            sign +
            (prop.tempBasalAdjustment.onehour - 100) +
            "%)",
        });
      } else {
        info.push({
