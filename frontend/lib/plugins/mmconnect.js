...
function init(env, entries, devicestatus, bus) {
  if (
    ...
    env.extendedSettings.mmconnect.userName &&
    env.extendedSettings.mmconnect.password
  ) {
...
function getOptions(env) {
  return {
    ...
    interval: parseInt(
      env.extendedSettings.mmconnect.interval || 60 * 1000,
      10,
    ),
    maxRetryDuration: parseInt(
      env.extendedSettings.mmconnect.maxRetryDuration || 32,
      10,
    ),
...
function makeHandler_(entries, devicestatus, sgvLimit, storeRawData) {
  ...
  return function handleCarelinkData(err, data) {
    if (err) {
      ...
      if (
        storeRawData &&
        (transformed.entries.length || transformed.devicestatus.length)
      ) {
...
function rawDataEntry(data) {
  ...
  cleansed["firstName"] =
    cleansed["lastName"] =
    cleansed["medicalDeviceSerialNumber"] =
      "<redacted>";

  // trim the default 288 sgvs returned by carelink
  if (cleansed["sgs"] && cleansed["sgs"] instanceof Array) {
    cleansed["sgs"] = cleansed["sgs"].slice(
      Math.max(0, cleansed["sgs"].length - 6),
    );
  }
