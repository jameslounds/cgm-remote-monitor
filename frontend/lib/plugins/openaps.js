...
function init(ctx) {
  ...
  openaps.getPrefs = function getPrefs(sbx) {
    ...
    var prefs = {
      ...
      predACOBColor: settings.predAcobColor
        ? settings.predAcobColor
        : "#FB8C00",
    ...
    _.forEach(recentData, function eachStatus(status) {
      ...
      if (
        ...
        if (
          ...
          result.lastPredBGs = _.isArray(enacted.predBGs)
      ...
      if (
        ...
        if (
          ...
          result.lastPredBGs = _.isArray(suggested.predBGs)
  ...
  openaps.checkNotifications = function checkNotifications(sbx) {
    ...
    if (level >= levels.WARN) {
      sbx.notifications.requestNotify({
        ...
        message:
          "Last Loop: " + utils.formatAgo(prop.lastLoopMoment, now.valueOf()),
        pushoverSound: "echo",
        group: "OpenAPS",
        plugin: openaps,
        debug: prop,
  ...
  openaps.findOfflineMarker = function findOfflineMarker(sbx) {
    return _.findLast(sbx.data.treatments, function match(treatment) {
      ...
      var eventEnd = treatment.duration
        ? eventTime + times.mins(treatment.duration).msecs
        : eventTime;
      return (
        eventTime <= sbx.time &&
  ...
  openaps.updateVisualisation = function updateVisualisation(sbx) {
      ...
      if (prop.lastPredBGs) {
        if (prop.lastPredBGs.values) {
          points = points.concat(
            _.map(prop.lastPredBGs.values, toPoints(0, "Values")),
          );
        }
        if (prop.lastPredBGs.IOB) {
          points = points.concat(
            _.map(prop.lastPredBGs.IOB, toPoints(3333, "IOB")),
          );
        }
        if (prop.lastPredBGs.ZT) {
          points = points.concat(
            _.map(prop.lastPredBGs.ZT, toPoints(4444, "Zero-Temp")),
          );
        }
        if (prop.lastPredBGs.aCOB) {
          points = points.concat(
            _.map(prop.lastPredBGs.aCOB, toPoints(5555, "Accel-COB")),
          );
        }
        if (prop.lastPredBGs.COB) {
          points = points.concat(
            _.map(prop.lastPredBGs.COB, toPoints(7777, "COB")),
          );
        }
        if (prop.lastPredBGs.UAM) {
          points = points.concat(
            _.map(prop.lastPredBGs.UAM, toPoints(9999, "UAM")),
          );
        }
      }
    ...
    if ("enacted" === prop.status.code) {
      ...
      var valueParts = [
        ...
        prop.lastEnacted.mealAssist && _.includes(selectedFields, "meal-assist")
  ...
  function virtAsstForecastHandler(next, slots, sbx) {
    ...
    if (lastEventualBG) {
      ...
      next(
        translate("virtAsstTitleOpenAPSForecast"),
  ...
  function virtAsstLastLoopHandler(next, slots, sbx) {
    ...
    if (lastLoopMoment) {
      var response = translate("virtAsstLastLoop", {
        params: [moment(lastLoopMoment).from(moment(sbx.time))],
      });
