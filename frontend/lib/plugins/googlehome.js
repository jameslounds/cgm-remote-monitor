...
function init() {
  ...
  googleHome.configureIntentHandler = function configureIntentHandler(
    intent,
    handler,
    metrics,
  ) {
  ...
  googleHome.getIntentHandler = function getIntentHandler(intentName, metric) {
    console.log(
      "Looking for handler for intent '" +
        intentName +
        "' for metric '" +
        metric +
        "'",
  ...
  googleHome.getRollup = function (rollupGroup, sbx, slots, locale, callback) {
    ...
    async.parallelLimit(nHandlers, 10, function (err, results) {
      ...
      callback(
        _.map(_.orderBy(results, ["priority"], ["asc"]), "results").join(" "),
      );
    });
  };

  // This creates the expected Google Home response
  googleHome.buildSpeechletResponse = function buildSpeechletResponse(
    output,
    expectUserResponse,
  ) {
    return {
