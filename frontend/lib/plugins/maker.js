...
function init(env) {
  var keys =
    ...
    env.extendedSettings.maker &&
    env.extendedSettings.maker.key &&
    env.extendedSettings.maker.key.split(" ");

  var announcementKeys =
    (env.extendedSettings &&
      env.extendedSettings.maker &&
      env.extendedSettings.maker.announcementKey &&
      env.extendedSettings.maker.announcementKey.split(" ")) ||
    keys;
  ...
  maker.makeRequests = function makeRequests(event, callback) {
    ...
    function sendByLevelAndName(callback) {
      maker.makeKeyRequests(
        event,
        "ns" + ((event.level && "-" + event.level) || "") + "-" + event.name,
        callback,
      );
    }
  ...
  maker.makeKeyRequest = function makeKeyRequest(
    key,
    event,
    eventName,
    callback,
