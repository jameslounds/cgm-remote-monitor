...
function init(env, cb, forceNewConnection) {
  function maybe_connect(cb) {
    if (mongo.db != null && !forceNewConnection) {
      ...
      if (!env.storageURI) {
        throw new Error(
          "MongoDB connection string is missing. Please set MONGODB_URI environment variable",
        );
      }
      ...
      const connect_with_retry = async function (i) {
        ...
        try {
          ...
          if (roles.length > 0 && roles[0].role == "readAnyDatabase") {
            ...
            cb(
              new Error(
                "MongoDB connection is in read only mode! Go back to MongoDB configuration and check your database user has read and write access.",
              ),
              null,
            );
          }
          ...
          if (err.message && err.message.includes("AuthenticationFailed")) {
            ...
            cb(
              new Error(
                "MongoDB authentication failed! Double check the URL has the right username and password in MONGODB_URI.",
              ),
              null,
            );
            return;
          ...
          if (err.name && err.name === "MongoServerSelectionError") {
            ...
            console.log(
              "Error connecting to MongoDB: %j - retrying in " +
                timeout / 1000 +
                " sec",
              err,
            );
