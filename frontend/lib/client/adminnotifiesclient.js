...
function init(client, $) {
  ...
  function wrapmessage(title, message, count, ago, persistent) {
    let html =
      "<hr><p><b>" +
      title +
      '</b></p><p class="adminNotifyMessage">' +
      message +
      "</p>";
    ...
    if (count > 1)
      additional += client.translate("Event repeated %1 times.", count) + " ";
    ...
    if (ago == 0) {
      ago = client.translate("less than 1");
    }
    if (!persistent && ago)
      additional += client.translate("Last recorded %1 %2 ago.", ago, units);

    if (additional)
      html +=
        '<p class="adminNotifyMessageAdditionalInfo">' + additional + "</p>";
    return html;
  }
