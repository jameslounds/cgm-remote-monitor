...
function init(client, serverSettings, $) {
  ...
  function updateBolusRender() {
    ...
    if (
      _.isNumber(bolusSettings.renderOver) &&
      bolusSettings.renderOver > 0 &&
      bolusSettings.renderOver < Number.MAX_SAFE_INTEGER
    ) {
      allRenderOverOptions.push(_.toNumber(bolusSettings.renderOver));
    }
    var sortedRenderOverOptions = _.chain(allRenderOverOptions)
      .uniq()
      .sort()
      .reverse()
      .value();
    ...
    $("#bolusRenderFormat").val(
      bolusSettings.renderFormat ? bolusSettings.renderFormat : "default",
    );
  ...
  function loadForm() {
    ...
    function appendThresholdValue(threshold) {
      return settings.alarmTypes.indexOf("simple") === -1
        ? ""
    ...
    $("#alarm-urgenthigh-browser")
      ...
      .text(
        translate("Urgent High Alarm") +
          appendThresholdValue(settings.thresholds.bgHigh),
      );
    $("#alarm-high-browser")
      ...
      .text(
        translate("High Alarm") +
          appendThresholdValue(settings.thresholds.bgTargetTop),
      );
    $("#alarm-low-browser")
      ...
      .text(
        translate("Low Alarm") +
          appendThresholdValue(settings.thresholds.bgTargetBottom),
      );
    $("#alarm-urgentlow-browser")
      ...
      .text(
        translate("Urgent Low Alarm") +
          appendThresholdValue(settings.thresholds.bgLow),
      );
    ...
    $("#basalrender").val(
      settings.extendedSettings.basal
        ? settings.extendedSettings.basal.render
        : "none",
    );
      ...
      if (client.plugins.specialPlugins.indexOf(plugin.name) > -1) {
        ...
        var dd = $(
          '<dd><input type="checkbox" id="' +
            ...
            id +
            '">' +
            translate(plugin.label || plugin.name) +
            "</label></dd>",
        );
        showPluginsSettings.append(dd);
        dd.find("input").prop(
          "checked",
          settings.showPlugins.indexOf(plugin.name) > -1,
        );
    ...
    $("#editprofilelink").toggle(
      settings.isEnabled("iob") ||
        settings.isEnabled("cob") ||
        settings.isEnabled("bwp") ||
        settings.isEnabled("basal"),
    );
  ...
  function wireForm() {
    ...
    $("#save").click(function (event) {
      function checkedPluginNames() {
        var checkedPlugins = [];
        $("#show-plugins input:checked").each(
          function eachPluginCheckbox(index, checkbox) {
      ...
      storeInBrowser({
        ...
        alarmTimeagoWarnMins:
          parseInt($("#alarm-timeagowarnmins-browser").val()) || 15,
        alarmTimeagoUrgent: $("#alarm-timeagourgent-browser").prop("checked"),
        alarmTimeagoUrgentMins:
          parseInt($("#alarm-timeagourgentmins-browser").val()) || 30,
        ...
        timeFormat: parseInt(
          $("input:radio[name=timeformat-browser]:checked").val(),
        ),
  ...
  function showLocalstorageError() {
    var msg =
      "<b>Settings are disabled.</b><br /><br />Please enable cookies so you may customize your Nightscout site.";
  ...
  function handleStorageVersions() {
    ...
    if (isNaN(previousVersion)) {
      ...
      if (settings.isEnabled("careportal")) {
        ...
        if (
          storedShowPlugins &&
          storedShowPlugins.indexOf("careportal") === -1
        ) {
  ...
  try {
    ...
    settings.extendedSettings.bolus.renderOver =
      bolusStored !== null
        ? _.toNumber(bolusStored.renderOver)
        : settings.extendedSettings.bolus.renderOver;
    settings.extendedSettings.bolus.renderFormat =
      bolusStored !== null
        ? bolusStored.renderFormat
        : settings.extendedSettings.bolus.renderFormat;
    settings.extendedSettings.bolus.renderFormatSmall =
      bolusStored !== null
